...
print(res.min_margin)  # 0.5 — distance to the nearest rounding edge, in grid units
```

`min_margin` is a stability diagnostic, never part of the hash. The identifier
is invariant under rigid motion and relabeling in exact arithmetic, but a
quantized value sitting on a rounding edge can be moved by float64 round-off
alone. Because the quantized values are a function of the geometry, so is the
margin: it is fixed for a given `(geometry, precision, method)`, so a corpus can
be screened before its identifiers are deposited. Compare it against your
coordinate-noise floor divided by `precision`; `0.0` is maximally fragile and
`0.5` is maximally safe. Idealized and symmetrized geometries are the usual
offenders at coarse grids — an ideal D6h benzene at a 0.01 Å grid has
`1.39·cos 60° = 0.695 Å` exactly on an edge.
